...
| 方案 | 结果 |
| :--- | :--- |
| `LayoutParams.setTouchableRegion`（只在不规则区域接收触摸） | 公开 SDK 里**不存在**，是 `@hide` 内部 API（已在 `android-36` 的 `android.jar` 里逐字节确认） |
| 用多个矩形拼接近似人物轮廓 | 实测最多只能腾出 18% —— 人物在画布上几乎铺满（底部整行不透明） |
| 手势中途切换 `FLAG_NOT_TOUCHABLE` | 系统不会重新派发，那一下只会消失 |
| 按住立绘 + 另一根手指点桌面 | 技术上可行，但属于非常规操作，容易误触，还可能出现窗口卡在「点不动」的状态 |

**当前做法**：整个去掉穿透功能。立绘区域整块接收触摸，
不会出现「点不动」的状态，也没有半透明鬼影。
