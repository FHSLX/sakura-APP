...
点立绘的透明区域应该点到**桌面图标**，而不是被悬浮窗吃掉。

原理：悬浮窗用 `WindowManager` 的 `FLAG_NOT_TOUCHABLE` 决定整窗是否接收触摸
（原生方法 `SakuraNative.setTouchable`）。所以流程是：

```
轻点在立绘透明处 → setTouchable(false) → 这一下落到桌面
                  → 140ms 后 setTouchable(true) 自动恢复
```

### 为什么原来完全没生效

**原生能力早就写好了，但 JS 侧从来没有调用过 `setTouchable`** ——
全文件搜不到一处调用，那个能力一直闲置。
...
2. **`portraitAlphaAt(x, y) < 16`** —— 立绘 PNG 自带透明通道，用 1×1 的
   canvas 取样判断。要注意 `object-fit: contain` + `object-position: bottom center`
   的换算，否则取样坐标会偏。

**不要**再补一个 `elementFromPoint` 兜底：立绘区域本身就包含 `#stage` 这类容器，
再问一次会把本该穿透的位置判成「有元素」而挡掉。

### 两个必须注意的坑

**恢复触摸的定时器一定要能独立兜底。** 窗口一旦停留在不可触摸状态，
网页就再也收不到任何事件，等于彻底失去控制。所以恢复只依赖 `setTimeout`，
不依赖任何后续事件，另外每 2 秒再做一次保险 `setTouchable(true)`。

**不要累积 `pointermove` 的最大位移来判断「是不是拖动」。**
指针拖远再拖回来时那个历史值仍然很大，会把之后的轻点**全部**误判成拖动 ——
这正是我第一版「点了没反应」的原因。只比较 `pointerdown` → `pointerup` 的位移即可。

### 实测（CDP 逐点验证）

| 点击位置 | 结果 |
| --- | --- |
| 对话框 / 输入框 / 发送 / ＋按钮 | blocked（正确） |
| 立绘透明处（alpha=0） | passed-through（正确） |
